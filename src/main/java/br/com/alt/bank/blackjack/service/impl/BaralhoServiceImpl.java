...
public class BaralhoServiceImpl<T> implements BaralhoService {
    ...
    public ResponseJogadorDTO iniciarJogo(RequestJogadorDTO dados) throws BlackJackExeption {
        ...
        if (!validaCartaAs(dados.getValorA())) {
        ...
        salvarUsuario(rodada, dados.getNome().toUpperCase(), dados.getValorA());
        salvarUsuario(rodada, MESA, null);
        salvarCarta(rodada, dados.getValorA());
    ...
    public String selecionarCarta(RequestSelecionarCartaDTO dados) throws BlackJackExeption {
        ...
        Carta carta = cartaRepository.findByPosicaoAndJogadorIsNull(dados.getCarta());
        Jogador jogador = jogadorRepository.findByNome(dados.getNomeJogador().toUpperCase());
        if (!validarJogador(dados.getNomeJogador())) {
            montarException("Favor digite um jogador valido!!!");
        }
        if (!validaObjetoNulo((T) jogador)) {
            montarException("Jogador " + dados.getNomeJogador() + " não encontrado!!!");
        }
        if (!validaObjetoNulo((T) carta)) {
            montarException("Carta " + dados.getCarta() + " não encontrado!!!");
        }
        if (!validaObjetoNulo((T) rodada)) {
            montarException("Jogo não encontrado!!!");
        }
        log.warn("Realizando buscar da cartas para Jogador " + dados.getNomeJogador());
    ...
    private Jogador salvarUsuario(Rodada rodada, String name, Integer valorA) {
        ...
        jogador.setValorAs(valorA);
        return jogadorRepository.save(jogador);
    }

    private Iterable<Carta> salvarCarta(Rodada rodada, Integer valorAs) {
        return cartaRepository.saveAll(criarBaralho(rodada, valorAs));
    }
    ...
    private String encerrarJogo(Rodada rodada, Jogador mesa, Jogador jogador) {
        if (validaObjetoNulo((T) mesa) && validaObjetoNulo((T) jogador)) {
            ...
            if (rodada.getRodadas().equals(VALOR_5)
                ...
                if(mesa.getPonto() > VALOR_21 && jogador.getPonto() > VALOR_21){
                    return "Jogo finalizando ambos os jogadores excederam 21 pontos";
                }else if (mesa.getPonto() > jogador.getPonto()) {
    ...
    private String retornarVencedor(Rodada rodada, Jogador Vencedor, Jogador Perderdor) {
        ...
        rodada.setNomePerdedor(Perderdor.getNome());
